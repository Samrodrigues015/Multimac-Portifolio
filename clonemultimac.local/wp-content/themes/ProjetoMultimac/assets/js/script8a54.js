...
(function () {
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function toggleFocus(event) {
      ...
      if (event.type === "touchstart") {
        ...
        if (menuItem && menuItem.parentNode) {
          menuItem.parentNode.querySelectorAll("li").forEach((link) => {
    ...
    // ===== Xopvision Layout =====
    function initXopvisionMobileScroll() {
      if (window.innerWidth <= 768) {
        const grid = document.getElementById("xopvisionIndustriesGrid");
        const dots = document.querySelectorAll(".xopvision-scroll-dot");
        if (!grid || !dots.length) return;

        grid.addEventListener("scroll", function () {
          const scrollLeft = grid.scrollLeft;
          const cardWidth = grid.firstElementChild?.offsetWidth || 300;
    ...
    function updateXopvisionLayout() {
      if (document.getElementById("xopvisionIndustriesGrid")) {
        initXopvisionMobileScroll();
      }
    }

    updateXopvisionLayout();
    window.addEventListener("resize", updateXopvisionLayout);
  });
})();
...
document.querySelectorAll(".faq-question").forEach((question) => {
  question.addEventListener("click", () => {
    ...
    const toggle = question.querySelector(".faq-toggle svg");
